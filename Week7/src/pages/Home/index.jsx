...
import { recipeCollections } from '../../data/recipes';

const recipePath = (slug) => `/cooking-guide/${slug}`;

const Home = () => {
  const { openCreateAccount } = useOutletContext();
    ...
    <div className="bg-gray-light min-h-screen pb-20">
        ...
        <div className="relative rounded-2xl overflow-hidden h-[450px]">
          ...
          <div className="absolute top-20 left-10 md:left-20 bg-white p-8 rounded-2xl shadow-xl max-w-sm">
            ...
            <div className="flex items-center gap-3 mb-6">
              <img src="/image/avatar.png" alt="Salad Caprese" className="w-8 h-8 rounded-full" />
              <span className="text-sm font-medium">Salad Caprese</span>
            </div>
            <button onClick={openCreateAccount} className="btn-primary w-full gap-2 font-bold py-3.5">
              Get Started <span className="text-lg">&rarr;</span>
            </button>
          </div>
        </div>
      </section>
      ...
      <section className="container-custom mt-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {recipeCollections.summer.map((recipe) => (
            <RecipeCard key={recipe.slug} {...recipe} to={recipePath(recipe.slug)} />
          ))}
        </div>
      </section>
      ...
      <section className="container-custom mt-20">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {recipeCollections.videos.map((recipe) => (
            <RecipeCard key={recipe.slug} {...recipe} to={recipePath(recipe.slug)} />
          ))}
        </div>
      </section>
      ...
      <section className="container-custom mt-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="flex bg-white rounded-2xl p-4 gap-6 hover:shadow-lg transition-all border border-transparent hover:border-pink-100">
            <img src="/image/visily_image_108.png" alt="Stuffed sticky rice ball" className="w-48 h-48 object-cover rounded-xl shrink-0" />
            <div className="flex flex-col justify-between py-2">
              <div>
                ...
                <div className="flex items-center gap-2 mt-4">
                  <img src="/image/avatar.png" alt="Jennifer King" className="w-8 h-8 rounded-full" />
          ...
          <div className="flex bg-white rounded-2xl p-4 gap-6 hover:shadow-lg transition-all border border-transparent hover:border-pink-100">
            <img src="/image/visily_image_93.png" alt="Strawberry smoothie" className="w-48 h-48 object-cover rounded-xl shrink-0" />
            <div className="flex flex-col justify-between py-2">
              <div>
                <h3 className="text-xl font-bold mb-1">Strawberry smoothie</h3>
                <span className="text-xs text-gray-400">42 minutes</span>
                <div className="flex items-center gap-2 mt-4">
                  <img src="/image/avatar.png" alt="Matthew Martinez" className="w-8 h-8 rounded-full" />
